...
import {StyleSheet,ScrollView, Dimensions,AsyncStorage, View, Alert,
...
import {Text,FormLabel, FormInput, Button, SocialIcon, Icon} from 'react-native-elements';
  ...
  render() {
    return (
      ...
      {this.state.loading ? <LoadingDataDialog /> : null }
        ...
        <ScrollView contentContainerstyle={{ flex: 1, justifyContent: 'flex-start', alignItems: 'center', padding : 20 }}>
        <View style = {styles.listItem}>
          <View>
            <Text style = {{fontSize : 18}}>User data</Text>
            <Text style = {{fontSize : 11}}>Edit user data</Text>
          </View>
          <Icon color = {!this.state.userDataModalVisible ? "green" : "gray"} name= {!this.state.userDataModalVisible ? "person" : "person-outline"}
              onPress = {() => this.setState({userDataModalVisible : !this.state.userDataModalVisible, errorDialogVisible : false})} />
        </View>
        ...
        <View style = {styles.userDataView}>
            ...
            <FormInput placeholder = "Your Username" inputStyle = {{width : width - 100 }} ref={usernameRef => this.usernameRef = usernameRef}
             onChangeText = {(username) => this.setState({username :  username, errorDialogVisible : false,passwordModalVisible : false, userDataModalVisible : false})}
            ...
            <FormInput placeholder = "Your first name" inputStyle = {{width : width - 100 }} ref={firstnameRef => this.firstnameRef = firstnameRef}
             onChangeText = {(firstname) => this.setState({firstname :  firstname, errorDialogVisible : false,passwordModalVisible : false, userDataModalVisible : false})}
            ...
            <FormInput placeholder = "Your last name" inputStyle = {{width : width - 100 }} ref={lastnameRef => this.lastnameRef = lastnameRef}
             onChangeText = {(lastname) => this.setState({lastname :  lastname, errorDialogVisible : false,passwordModalVisible : false, userDataModalVisible : false})}
          ...
          <View style = {styles.listItem}>
            ...
            <Icon color = {this.state.passwordModalVisible ? "green" : "gray"} name= {this.state.passwordModalVisible ? "lock" : "lock-outline"}
                onPress = {() => this.setState({passwordModalVisible : !this.state.passwordModalVisible, errorDialogVisible : false})} />

          </View>
          { !this.state.passwordModalVisible ? null :
          <View style = {styles.userDataView}>
            <View style = {styles.inputView}>
              <FormInput placeholder = "Insert new password" inputStyle = {{ width :  width * 0.75}}  ref={passwordRef => this.passwordRef = passwordRef}
            ...
            <View style = {styles.inputView}>
              <FormInput placeholder = "Repeat new password " inputStyle = {{ width :  width * 0.75}}  ref={rPasswordRef => this.rPasswordRef = rPasswordRef}
            ...
            <View style = {styles.inputView}>
              <FormInput placeholder = "Insert your old password" inputStyle = {{ width :  width * 0.75}}  ref={oldPasswordRef => this.oldPasswordRef = oldPasswordRef}
          ...
          <View style = {styles.listItem}>
            ...
            <Icon color = "red" name='power-settings-new' onPress = {this.logoutUser.bind(this)} />
          </View>
